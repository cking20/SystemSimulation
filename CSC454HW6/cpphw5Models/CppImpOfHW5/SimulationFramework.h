...
std::vector<OutputTimePairs<O>*> SimulationFramework<I,O>::RunEventSimulation(){
    ...
    //std::cout << "num input Times: " << theCmdsTime.size() << std::endl;
    for(int i = 0; i < theCmdsTime.size();i++){
        //std::cout <<"i="<<i<<" Time: "<<theCmdsTime.at(i)->getRealTime()<<", "<<theCmdsTime.at(i)->getDiscreteTime()<< std::endl;
    }
    ...
    //std::cout << "Output should be :" << theoutput.at(0)->getData() << std::endl;
    return theoutput;
}
