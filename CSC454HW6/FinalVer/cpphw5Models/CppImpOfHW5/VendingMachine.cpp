...
std::vector<std::string> VendingMachine::Tick(std::vector<std::string> inputs){
    std::vector<std::string> out = OutPut();
    StateTransition(inputs);
    return out;
}

std::vector<std::string> VendingMachine::OutPut(){
    std::string output = "";
        if(cancelPushed){            
...
std::vector<std::string> VendingMachine::SplitString(std::string line, std::string delim){
    size_t pos = 0;
