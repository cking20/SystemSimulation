...
void Mem::StateTransitionCon(std::vector<std::string>args){
    headEmpty = false;
    tailEmpty = false;
    qHead = qTail;
    qTail = std::stoi(args.at(0));
}
void Mem::StateTransitionExt(std::vector<std::string>args, double e){
    headEmpty = false;
    tailEmpty = false;
    qHead = qTail;
    qTail = std::stoi(args.at(0));
}
void Mem::StateTransitionInt(){
    if(!tailEmpty){
        tailEmpty = true;
    } else {headEmpty = true;}
    return;
