...
void LoadCmdsFromFile(SimulationFramework<vector<string>,vector<string>> *fp, string fileName){
    ifstream ifs;
    ifs.open(fileName);
    if(ifs.is_open()){
        while (!ifs.eof()){
            ...
            fp->AddInput(t.getRealTime(), t.getDiscreteTime(), inputs);
        }
    }
    ifs.close();
}

void PerformUserAction(vector<string> inp, SimulationFramework<vector<string>,vector<string>> * fp){
    ...
    if(inp.at(0).compare("-run") == 0){
        fp->paused = false;
        vector<OutputTimePairs<vector<string>>*> out(fp->RunEventSimulation());
        cout << "num outputs gotten: " << out.size() << endl;
        string comb = "";
        OutputTimePairs<vector<string>>* temp;
        
        for (int i = 0; i < out.size(); i++) {
            temp = out.at(i);
            //String[] d = temp.getData();
            vector<string> d(temp->getData());
            //if (d != NULL) {
...
/*
void OutputRun(SimulationFramework<vector<string>,vector<string>> *fp){
    while(keepRunning){
        if(fp->producedOutput){
            vector<vector<string>> outputs = fp->FlushOutputs();
            for(int i =0; i < outputs.size(); i++){
...
void RunSim(SimulationFramework<vector<string>,vector<string>> *fp){
    cout << "running"<<endl;
    fp->Run();
    cout << "run thread done " << endl;
}
 */
EventNetwork<vector<string>,vector<string>>* BuildXORNet(){
    EventNetwork<vector<string>,vector<string>>* theNetwork = 
            new EventNetwork<vector<string>,vector<string>>();
...
int main(int argc, char** argv) {
    ...
    string inputLine;
    vector<string> inputs;    
    SimulationFramework<vector<string>,vector<string>> *f = new SimulationFramework<vector<string>,vector<string>>(true,true,true, true,5000);
    EventNetwork<vector<string>,vector<string>>* theNet = BuildXORNet();
    ...
    do{
        try{           
            ...
            if(inputs[0].at(0) == '-')
                ...
                f->AddInput(t.getRealTime(), t.getDiscreteTime(), inputs);
